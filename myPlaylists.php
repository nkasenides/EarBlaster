...
<meta charset="utf-8">
<title>My Playlists - EarBlaster</title>
<?php 
...
<div class="content">
    ...
    <?php 
		if (isset($_GET["status"])) {
			$status = $_GET["status"];
			if ($status == "error") {
				echo '<p class="error"> Could not delete playlist. </p>';
			}//fail
			else if ($status == "ok") {
				echo '<p class="success"> Playlist deleted. </p>';
			}//success
			else if ($status == "created") {
				echo '<p class="success"> Playlist created. </p>';
			}//created
		}//end is isset()
	?>
      
    <div class="trackBlock">
    	<h2><?php include("php/getUsername.php");?>'s Playlists</h2>
		<?php include("php/displayAllPlaylists.php"); ?>
        <a class="buttonGreen" href="addPlaylist.php?user=<?php echo $_SESSION["userid"];?>">Add a playlist</a>
    </div><!--end trackblock>
